...
//set up Router.
  ...
  requestNotes = () => {
    axios.get('http://192.168.1.8:3002/')
      .then(response => {
  ...
  handleNewNote = e => {
    ...
    axios.post('http://192.168.1.8:3002/', newNote)
      .then(response => {
        this.updateNoteState(response.data);
  ...
  render() {
    ...
    if (this.state.formShowMe) {
      showForm =
        <div>
          <NoteForm
            ...
            logEventInfo={this.logEventInfo}
      ...
      showForm = <div><div className="new-note-box">
        <button type="button" onClick={() => this.switchFormView()}>
